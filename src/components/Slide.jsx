...
export const Slide = ({ image, heading, subheading, description, buttonText, link }) => {
  ...
  const cldImage = cld
    .image(image) // e.g. 'banners/slide1'
    .format('auto')
    .quality('auto')
    .resize(fill().width(1920).height(800).gravity(autoGravity()));

  return (
    <div className='w-full relative h-[600px] lg:h-[800px] overflow-hidden'>
      <AdvancedImage cldImg={cldImage} className='w-full h-full object-cover' />
      <div className="absolute inset-0 bg-[rgba(0,0,0,0.5)] bg-opacity-60 flex flex-col items-center justify-center text-white text-center p-4">
